...
static mrb_value str_replace(mrb_state *mrb, struct MRString *s1, struct MRString *s2);
static mrb_value mrb_str_subseq(mrb_state *mrb, mrb_value str, mrb_int beg, mrb_int len);

MRB_API mrb_int
mrb_str_strlen(mrb_state *mrb, struct MRString *s)
{
...
static inline void
resize_capa(mrb_state *mrb, struct MRString *s, mrb_int capacity)
{
  if (RSTR_EMBED_P(s)) {
    if (MRSTRING_EMBED_LEN_MAX < capacity) {
...
MRB_API void
mrb_str_modify(mrb_state *mrb, struct MRString *s)
{
  if (RSTR_SHARED_P(s)) {
...
#define mrb_obj_alloc_string(mrb) ((struct MRString*)mrb_obj_alloc((mrb), MRB_TT_STRING, (mrb)->string_class))

static struct MRString*
...
static inline void
str_with_class(mrb_state *mrb, struct MRString *s, mrb_value obj)
{
...
{
  struct MRString *s = str_new(mrb, 0, 0);
...
static void
str_buf_cat(mrb_state *mrb, struct MRString *s, const char *ptr, size_t len)
{
  size_t capa;
  size_t total;
  ptrdiff_t off = -1;
...
void
mrb_gc_free_str(mrb_state *mrb, struct MRString *str)
{
  if (RSTR_EMBED_P(str))
    /* no code */;
...
mrb_str_to_cstr(mrb_state *mrb, mrb_value str0)
{
  ...
  s = str_new(mrb, MRSTRING_PTR(str0), MRSTRING_LEN(str0));
...
static void
str_make_shared(mrb_state *mrb, struct MRString *s)
{
  if (!RSTR_SHARED_P(s)) {
...
{
  struct MRString *s1 = mrb_str_ptr(self), *s2;
  mrb_int len;
...
{
  struct MRString *s = mrb_str_ptr(a);
  struct MRString *s2 = mrb_str_ptr(b);
  struct MRString *t;
...
mrb_str_times(mrb_state *mrb, mrb_value self)
{
  ...
  if (times && MRB_INT_MAX / times < MRSTRING_LEN(self)) {
    mrb_raise(mrb, E_ARGUMENT_ERROR, "argument too big");
  }

  len = MRSTRING_LEN(self)*times;
  str2 = str_new(mrb, 0, len);
  str_with_class(mrb, str2, self);
  p = RSTR_PTR(str2);
  if (len > 0) {
    n = MRSTRING_LEN(self);
    memcpy(p, MRSTRING_PTR(self), n);
    while (n <= len/2) {
      memcpy(p + n, p, n);
      n *= 2;
    }
...
mrb_str_cmp(mrb_state *mrb, mrb_value str1, mrb_value str2)
{
  mrb_int len;
  mrb_int retval;
  struct MRString *s1 = mrb_str_ptr(str1);
  struct MRString *s2 = mrb_str_ptr(str2);
...
{
  const mrb_int len = MRSTRING_LEN(str1);

  if (len != MRSTRING_LEN(str2)) return FALSE;
  if (memcmp(MRSTRING_PTR(str1), MRSTRING_PTR(str2), (size_t)len) == 0)
    return TRUE;
  return FALSE;
}
...
mrb_str_index(mrb_state *mrb, mrb_value str, mrb_value sub, mrb_int offset)
{
  ...
  sptr = MRSTRING_PTR(sub);
  slen = MRSTRING_LEN(sub);
  len = MRSTRING_LEN(str) - offset;
...
{
  struct MRString *s = mrb_str_ptr(str);
  struct MRString *dup = str_new(mrb, 0, 0);
...
num_index:
      ...
      if (!mrb_nil_p(str) && MRSTRING_LEN(str) == 0) return mrb_nil_value();
      return str;
...
mrb_str_chomp_bang(mrb_state *mrb, mrb_value str)
{
  ...
  newline = MRSTRING_PTR(rs)[rslen-1];
  if (rslen == 1 && newline == '\n')
    newline = MRSTRING_PTR(rs)[rslen-1];
  if (rslen == 1 && newline == '\n')
    goto smart_chomp;
  ...
  if (p[len-1] == newline &&
     (rslen <= 1 ||
     memcmp(MRSTRING_PTR(rs), pp, rslen) == 0)) {
...
mrb_str_substr(mrb_state *mrb, mrb_value str, mrb_int beg, mrb_int len)
{
  ...
  if (beg > MRSTRING_LEN(str)) return mrb_nil_value();
  if (beg < 0) {
    beg += MRSTRING_LEN(str);
    if (beg < 0) return mrb_nil_value();
  }
  if (beg + len > MRSTRING_LEN(str))
    len = MRSTRING_LEN(str) - beg;
  if (len <= 0) {
    len = 0;
  }
  return mrb_str_subseq(mrb, str, beg, len);
...
mrb_str_include(mrb_state *mrb, mrb_value self)
{
  ...
  if (mrb_fixnum_p(str2)) {
    include_p = (memchr(MRSTRING_PTR(self), mrb_fixnum(str2), MRSTRING_LEN(self)) != NULL);
  }
  else {
...
mrb_str_index_m(mrb_state *mrb, mrb_value str)
{
  ...
  switch (mrb_type(sub)) {
    case MRB_TT_FIXNUM: {
      ...
      unsigned char *p = (unsigned char*)MRSTRING_PTR(str);
...
static mrb_value
str_replace(mrb_state *mrb, struct MRString *s1, struct MRString *s2)
{
  long len;
...
mrb_str_reverse(mrb_state *mrb, mrb_value str)
{
  ...
  if (MRSTRING_LEN(str) <= 1) return mrb_str_dup(mrb, str);

  s2 = str_new(mrb, 0, MRSTRING_LEN(str));
  str_with_class(mrb, s2, str);
  s = MRSTRING_PTR(str); e = MRSTRING_END(str) - 1;
  p = RSTR_PTR(s2);
...
mrb_str_rindex_m(mrb_state *mrb, mrb_value str)
{
  ...
  mrb_value sub;
  mrb_value vpos;
  mrb_int pos, len = MRSTRING_LEN(str);
...
mrb_str_split_m(mrb_state *mrb, mrb_value str)
{
  ...
  if (argc == 0 || mrb_nil_p(spat)) {
    ...
    if (mrb_string_p(spat)) {
      split_type = string;
      if (MRSTRING_LEN(spat) == 1 && MRSTRING_PTR(spat)[0] == ' ') {
          split_type = awk;
      }
    }
    else {
  ...
  beg = 0;
  if (split_type == awk) {
    char *ptr = MRSTRING_PTR(str);
    char *eptr = MRSTRING_END(str);
    char *bptr = ptr;
    mrb_bool skip = TRUE;
    unsigned int c;
    ...
    char *ptr = MRSTRING_PTR(str); /* s->as.ary */
    char *temp = ptr;
    char *eptr = MRSTRING_END(str);
    mrb_int slen = MRSTRING_LEN(spat);
  ...
  if (MRSTRING_LEN(str) > 0 && (lim_p || MRSTRING_LEN(str) > beg || lim < 0)) {
    if (MRSTRING_LEN(str) == beg) {
      tmp = mrb_str_new_empty(mrb, str);
    }
    else {
      tmp = mrb_str_subseq(mrb, str, beg, MRSTRING_LEN(str)-beg);
    }
  ...
  if (!lim_p && lim == 0) {
    ...
    while ((len = RARRAY_LEN(result)) > 0 &&
           (tmp = RARRAY_PTR(result)[len-1], MRSTRING_LEN(tmp) == 0))
...
{
  struct MRString *ps = mrb_str_ptr(*ptr);
...
mrb_str_to_inum(mrb_state *mrb, mrb_value str, mrb_int base, mrb_bool badcheck)
{
  ...
  if (s) {
    ...
    if (s[len]) {    /* no sentinel somehow */
      struct MRString *temp_str = str_new(mrb, s, len);
      s = RSTR_PTR(temp_str);
    }
  }
...
mrb_str_dump(mrb_state *mrb, mrb_value str)
{
  ...
  const char *p, *pend;
  char *q;
  struct MRString *result;

  len = 2;                  /* "" */
  p = MRSTRING_PTR(str); pend = p + MRSTRING_LEN(str);
  while (p < pend) {
    unsigned char c = *p++;
    switch (c) {
      case '"':  case '\\':
...
{
  return mrb_str_cat(mrb, str, MRSTRING_PTR(str2), MRSTRING_LEN(str2));
}
...
mrb_str_inspect(mrb_state *mrb, mrb_value str)
{
  ...
  p = MRSTRING_PTR(str); pend = MRSTRING_END(str);
  for (;p < pend; p++) {
    unsigned char c, cc;
...
mrb_init_string(mrb_state *mrb)
{
  ...
  mrb_static_assert(MRSTRING_EMBED_LEN_MAX < (1 << 5), "pointer size too big for embedded string");
